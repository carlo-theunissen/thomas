...
def sentence_embedder(data, text, model): 
    """
...
def topic_modelling(data, model, vectorizer): #Check
    """
    ...
    topicdf = pd.DataFrame(temp, columns = ["topic" + str(i) for i in range(len(temp[0]))])
    data = data.join(topicdf)
    return data, topicdf
...
#-------------------- Modifying and splitting training data ----------------------------
    ...
    # Turn all text to lower text and create a new column which is a combination of title and abstract.
...
ohe_model = OneHotEncoder(handle_unknown="ignore").fit(x_train.venue_encoded.values.reshape(-1,1)) # Look at venue encoding!!!
tfidf_model_word =  TfidfVectorizer(max_df = .9, max_features = 120 ).fit(x_train["text"].to_numpy()) # Clean the text!
tfidf_model_ngrams =  TfidfVectorizer(max_df = .9, min_df = 4 ,max_features= 100 , ngram_range = (2,3)).fit(x_train["text"].to_numpy()) #Clean the text!
vector_model = CountVectorizer().fit(x_train["text"].to_numpy())
topic_model = LatentDirichletAllocation(n_components=100).fit(vector_model.transform(x_train["text"].to_numpy())) # Ik twijfel hierover die .fit moet steeds om de functie topic_model() generaliseerbaar te kunnen houden
...
x_validation_pure = x_validation.copy
...
x_train = x_train[x_train.columns.drop(list(x_train.filter(regex='ngram')))]
x_validation = x_validation[x_validation.columns.drop(list(x_validation.filter(regex='ngram')))]
...
clf.fit(x_train_piped, y_train_oversampled)
clf.score(x_train_piped, y_train_oversampled)
clf.score(x_validation_piped, y_validation)
...
for i, paper in enumerate(paperId):
    d = {"paperId" : paper, "authorId": str(predictions[i])}
    result.append(d)

with open('predicted_test.json', 'w') as outfile:
